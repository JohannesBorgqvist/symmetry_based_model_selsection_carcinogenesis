...
model_str = "IM-II"
# Fit the model to the data
R_hat_IM, opt_para_IM, SS_IM = fit_to_data.PE_risk_profiles(t,R,model_str,para_IM)
# Save the estimated parameters
#write_output.save_data_PE(data_str,model_str,opt_para_IM,SS_IM,alpha)
# FIT POWER LAW
# Define the model string entailing that we are concentrating on the exponential
# model
model_str = "PLM"
# Fit the model to the data
#print("Model 2")
R_hat_PLM, opt_para_PLM, SS_PLM = fit_to_data.PE_risk_profiles(t,R,model_str,para_pow)
...
# Define the string were the plot will be stored
#file_str = "../Figures/Fig1/Input/PLM.tex"
# Define the string defining the settings for the plot
...
#write_output.plot_LaTeX_2D(t,R_hat_PLM,file_str,plot_str,legend_str)
# Define the string were the plot will be stored
#file_str = "../Figures/Fig1/Input/IM-II.tex"
# Define the string defining the settings for the plot
...
#write_output.plot_LaTeX_2D(t,R_hat_IM,file_str,plot_str,legend_str)
# Pyplot
# Overall properties
...
#-----------------------------------------------------------------
# Allocate memory for the output vector
R_hat_IM = np.zeros(t_new.shape)
# Extract the optimal parameters
A_opt_IM = opt_para_IM[0]
tau_opt_IM = opt_para_IM[1]
# Now we calculate the original time series
for index in range(len(R_hat_IM)):
    R_hat_IM[index] = np.exp(fit_to_data.objective_mixed(t_new[index],A_opt_IM,tau_opt_IM,alpha))
...
# Define the plot
#write_output.plot_LaTeX_2D(t_new,R_hat_IM,file_str,plot_str,legend_str)
# Define a transformation parameter
#epsilon = 0.37
# This parameter is there purely for esthetical reasons...
#epsilon_delta = 0.025
...
# Transform the time series 
#t_trans,R_trans = symmetry_based_model_selection.IM_II_transformation(t_new[80],R_hat_IM[80],epsilon-epsilon_delta,tau_opt_IM,alpha)
...
#for index in range(81,len(t_new),1):
    # Transform the time series 
    #t_trans,R_trans = symmetry_based_model_selection.IM_II_transformation(t_new[index],R_hat_IM[index],epsilon-epsilon_delta,tau_opt_IM,alpha)
    # Save the transformed stuff
    #write_output.plot_LaTeX_2D(t_trans,R_trans,file_str,plot_str,legend_str_empty)
# Calculate the transformed solution
#t_hat_1 = []
...
#for index in range(len(t_new)):
    # Calculate the transformed values
    #t_hat, R_hat = symmetry_based_model_selection.IM_II_symmetry(t_new[index],R_hat_IM[index],epsilon,tau_opt_IM,alpha)
    # Append the transformed values
    #t_hat_1.append(t_hat)
    #R_IM_hat_1.append(R_hat)
# Define the string defining the settings for the plot
#plot_str = "color=mixed_2,line width=2pt,"
# Define the string with the legend
#legend_str = "IM-II transformed solution, $\hat{R}_1(\hat{t})$"
# Save the transformed stuff
#write_output.plot_LaTeX_2D(t_hat_1,R_IM_hat_1,file_str,plot_str,legend_str)
# Re-define plot string
#plot_str = "color=black,->,>=latex,densely dashed"
# Loop over and add all other transformations
#for index in range(80,len(t_new),1):
    # Transform the time series 
    #t_trans,R_trans = symmetry_based_model_selection.IM_II_transformation(t_hat_1[index],R_IM_hat_1[index],epsilon-epsilon_delta,tau_opt_IM,alpha)
    # Save the transformed stuff
    #write_output.plot_LaTeX_2D(t_trans,R_trans,file_str,plot_str,legend_str_empty)
# Calculate the transformed solution
#t_hat_2 = []
...
#for index in range(len(t_new)):
    # Calculate the transformed values
    #t_hat, R_hat = symmetry_based_model_selection.IM_II_symmetry(t_hat_1[index],R_IM_hat_1[index],epsilon,tau_opt_IM,alpha)
    # Append the transformed values
    #t_hat_2.append(t_hat)
    #R_IM_hat_2.append(R_hat)
# Define the string defining the settings for the plot
#plot_str = "color=mixed_3,line width=2pt,"
# Define the string with the legend
#legend_str = "IM-II transformed solution, $\hat{R}_2(\hat{t})$"
# Save the transformed stuff
#write_output.plot_LaTeX_2D(t_hat_2,R_IM_hat_2,file_str,plot_str,legend_str)
#-----------------------------------------------------------------
...
#epsilon_vec = np.linspace(0.0, 10.0, num=100)
...
#for eps_ind in range(len(epsilon_vec)):
    ...
    # Calculate the fit
    #R_hat_mixed_new, opt_para_mixed, SS_IM = fit_to_data.PE_risk_profiles(t_mixed_dat,np.log(R_mixed_dat),model_str,para_IM)
    # Append the fit
...
# Define the string were the plot will be stored
file_str = "../Figures/Fig3_panels/PLM_fit/Input/step_1.tex"
# Define the string defining the settings for the plot
#plot_str = "only marks,scatter,mark=halfcircle*,mark size=2.9pt,color=black"
plot_str = "only marks, mark=halfcircle*,mark size=1.5pt,color=black,"
# Define the string with the legend
...
# Define the string were the plot will be stored
file_str = "../Figures/Fig3_panels/PLM_fit/Input/step_2.tex"
# Define the plot
write_output.plot_LaTeX_2D(t_trans_data,R_trans_data,file_str,plot_str,legend_str)
...
# STEP 1: TRANSFORMED DATA
# Define the string were the plot will be stored
file_str = "../Figures/Fig3_panels/IM-II_fit/Input/step_1.tex"
# Define the string defining the settings for the plot
#plot_str = "only marks,scatter,mark=halfcircle*,mark size=2.9pt,color=black"
plot_str = "only marks, mark=halfcircle*,mark size=1.5pt,color=black,"
# Define the string with the legend
...
legend_str = "Action of symmetry $\\Gamma_{2}\\left(\\epsilon=" + str(epsilon).replace("e-0","\\cdot 10^{-") + "\\right)$"
# Transform the time series 
t_trans,R_trans = symmetry_based_model_selection.IM_II_transformation(t[48],R_ori_data[48],epsilon,tau_opt_IM,alpha)
# Define the plot string, defining the colour
plot_str = "color=black,->,>=latex,densely dashed"
# Save the transformed stuff
write_output.plot_LaTeX_2D(t_trans,R_trans,file_str,plot_str,legend_str)
...
for index in range(49,len(t),1):
    # Transform the time series 
    t_trans,R_trans = symmetry_based_model_selection.IM_II_transformation(t[index],R_ori_data[index],epsilon,tau_opt_IM,alpha)
    # Save the transformed stuff
    write_output.plot_LaTeX_2D(t_trans,R_trans,file_str,plot_str,legend_str_empty)
# Define the transformed data
R_trans_data = []
...
for index in range(len(R_ori_data)):
    # Calculate each transformed coordinate
    t_hat_temp, R_hat_temp = symmetry_based_model_selection.IM_II_symmetry(t[index],R_ori_data[index],epsilon,tau_opt_IM,alpha)
    # Save the transformed coordinates
    t_trans_data.append(t_hat_temp)
    R_trans_data.append(R_hat_temp)
plot_str = "only marks, mark=diamond*,mark size=1.5pt,color=gray,every mark/.append style={solid, fill=gray}"
...
# STEP 2: FIT TO TRANSFORMED DATA
# Define the string were the plot will be stored
file_str = "../Figures/Fig3_panels/IM-II_fit/Input/step_2.tex"
# Define the plot
#write_output.plot_LaTeX_2D(t_trans_data,R_trans_data,file_str,plot_str,legend_str)
# Define the logarithm of the data
R_trans_data_log = [np.log(R_trans) for R_trans in R_trans_data]
...
# Fit the model to the data
#print("Model 2")
R_hat_IM, opt_para_IM_II, SS_IM_II = fit_to_data.PE_risk_profiles(t_trans_data,R_trans_data_log,model_str,para_IM)
# Convert these to lists
R_hat_IM = R_hat_IM.tolist()
t_trans_data = t_trans_data.tolist()
# Take the exponential of R
R_hat_IM = [np.exp(R_hat) for R_hat in R_hat_IM]
# Define the string defining the settings for the plot
plot_str = "color=mixed_2,line width=2pt,"
# Define the string with the legend
legend_str = "Fitted IM-II, $R(t)$"
# Define the plot
write_output.plot_LaTeX_2D(t_trans_data,R_hat_IM,file_str,plot_str,legend_str)
# STEP 3: Inversely transform the model back
# Define the string were the plot will be stored
...
#write_output.plot_LaTeX_2D(t,R_ori_data,file_str,plot_str,legend_str)
#---------------------------------------------------------------------------------
# Append the optimal gamma value for the PLM
para_pow.append(gamma_opt_PLM)
# Append the optimal tau value for the IM-II
para_IM.append(tau_opt_IM)
# Define an epsilon vector

epsilon_vector = list(np.linspace(0.0,0.75,num=100))
# PLM: Calculate the relative fit
Delta_fit_pow = symmetry_based_model_selection.sym_model_sel(t,R,epsilon_vector,para_pow,"PLM")
# IM_II: Calculate the relative fit
Delta_fit_mixed = symmetry_based_model_selection.sym_model_sel(t,R,epsilon_vector,para_IM,"IM-II")
# Save the power law
write_output.plot_LaTeX_2D(epsilon_vector,Delta_fit_pow,"../Figures/Fig3_panels/model_sel/Input/PLM.tex","color=pow_2,line width=2pt,","PLM")
# Save the mixed
write_output.plot_LaTeX_2D(epsilon_vector,Delta_fit_mixed,"../Figures/Fig3_panels/model_sel/Input/IM_II.tex","color=mixed_2,line width=2pt,","IM-II")

#epsilon_test = [0]
#Delta_fit_test = symmetry_based_model_selection.sym_model_sel(t,R,epsilon_test,para_pow,"PLM")
